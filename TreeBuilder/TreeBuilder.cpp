...
void TreeBuilder::LoadDataSet(const Dataset &dataset) {
  ...
  params.SetMaxNums(dataset.Meta().wnum_samples, dataset.Meta().multiplier);
  if (params.cost_function == Entropy)
    Cost::ConstructNLogNTable(dataset.Meta().multiplier, dataset.Meta().wnum_samples + 1);
}

void TreeBuilder::UpdateDataSet() {
  params.SetMaxNums(dataset->Meta().wnum_samples, dataset->Meta().multiplier);
  if (params.cost_function == Entropy)
    Cost::ExtendNLogNTable(dataset->Meta().wnum_samples + 1);
}
...
void TreeBuilder::Init(uint32_t num_threads,
  ...
  if (params.cost_function == GiniImpurity)
    splitter_ptr.gini_splitter->Init(num_threads, dataset, params);
  if (params.cost_function == Entropy)
    splitter_ptr.entropy_splitter->Init(num_threads, dataset, params);
  if (params.cost_function == Variance)
    splitter_ptr.variance_splitter->Init(num_threads, dataset, params);
...
void TreeBuilder::Build(StoredTree &tree) {
  Init(1, tree);
  nodes[node_top++] = make_unique<TreeNode>(dataset);
...
const vector<uint32_t> &TreeBuilder::ShuffleFeatures(TreeNode *node) {
  uint32_t thread_id = ParallelTreeNodeNonMember::GetThreadId(0, node);
  vector<uint32_t> &feature_set = feature_sets[thread_id];
...
void TreeBuilder::Split(uint32_t feature_type,
  ...
  if (params.cost_function == GiniImpurity)
    splitter_ptr.gini_splitter->Split(feature_idx, feature_type, dataset, node);
  if (params.cost_function == Entropy)
    splitter_ptr.entropy_splitter->Split(feature_idx, feature_type, dataset, node);
  if (params.cost_function == Variance)
    splitter_ptr.variance_splitter->Split(feature_idx, feature_type, dataset, node);
  if (to_delete_sorted_idx)
    node->DiscardSortedIdx(feature_idx);
}
...
void TreeBuilder::InsertChildNodes(TreeNode *node) {
  ...
  nodes[left_child_id] = make_unique<TreeNode>(left_child_id, IsLeftChildType, node);
  nodes[right_child_id] = make_unique<TreeNode>(right_child_id, IsRightChildType, node);
